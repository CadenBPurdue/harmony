    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (status.isComplete) {
          console.log("[Homepage] Apple Music loading complete, refreshing playlists");
          clearInterval(pollInterval);
          
          // Refresh the playlists to get final data
          const updatedPlaylists = await window.electronAPI.getAppleMusicLibrary(true);
          ...
          if (selectedPlaylist && selectedPlaylist.origin === "Apple Music") {
            const updatedSelected = updatedPlaylists.find(p => 
  ...
  useEffect(() => {
    ...
    const handlePlaylistLoaded = (playlistInfo) => {
      ...
      if (playlistInfo.origin === "Apple Music") {
        ...
        if (selectedPlaylist && selectedPlaylist.playlist_id === playlistInfo.id) {
          ...
          {!appleMusicStatus.isComplete && appleMusicStatus.total > 0 ? (
            <Box sx={{ 
              ...
              <Typography variant="caption" sx={{ ml: 1.5, color: "white" }}>
                {Math.round((appleMusicStatus.loaded / appleMusicStatus.total) * 100)}%
              </Typography>
            </Box>
          ) : null}
          {appleMusicOpen ? 
  ...
  const renderPlaylistDetails = () => {
    ...
    return (
      <Box>
        ...
        >
          <Box sx={{ maxWidth: '70%' }}>  {/* Limit width to prevent overflow */}
        ...
        >
          <TableContainer sx={{ maxWidth: '100%' }}>
            <Table stickyHeader aria-label="playlist tracks table">
              <TableHead>
                <TableRow>
                  <TableCell width="5%" sx={{ bgcolor: 'rgba(134, 97, 193, 0.05)' }}>#</TableCell>
                  <TableCell width="35%" sx={{ bgcolor: 'rgba(134, 97, 193, 0.05)' }}>Song</TableCell>
                  <TableCell width="25%" sx={{ bgcolor: 'rgba(134, 97, 193, 0.05)' }}>Artist</TableCell>
                  <TableCell 
                    width="25%" 
                    sx={{ 
                  ...
                  trackItems.map((item, index) => {
                    ...
                    return (
                      <TableRow 
                        key={index}
                        sx={{
                          '&:nth-of-type(odd)': { bgcolor: 'rgba(134, 97, 193, 0.03)' },
        ...
        >
          ...
          >
            <Box sx={{ 
              ...
              <Typography variant="h4" color="primary.main" sx={{ fontWeight: 'bold', mr: 'auto' }}>
                Harmony
              </Typography>
