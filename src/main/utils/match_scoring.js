...
function isOriginalArtist(foundArtist, originalArtist) {
  ...
  if (
    foundNoAccents.includes(originalNoAccents) ||
    originalNoAccents.includes(foundNoAccents)
  )
    return true;
  ...
  if (
    foundFirstWord &&
    originalFirstWord &&
    foundFirstWord === originalFirstWord &&
    foundFirstWord.length > 1
  ) {
    return true;
  }
...
function calculateDurationScore(duration1, duration2) {
  ...
  if (durationDiffPercent <= 0.1) return 0.9; // Very close
...
function scoreSongMatch(candidate, searchSong, options = {}) {
  ...
  const albumScore = searchSong.album
    ? albumMatches
      ? 1
      : calculateSimilarity(candidate.album, searchSong.album)
    : 1;
  ...
  const finalScore =
    titleComparison.isExactMatch && artistMatches
...
function findExactMatch(candidates, searchSong) {
  ...
  for (const candidate of candidates) {
    ...
    if (
      areEquivalentTitles(candidate.details.name, searchSong.name) &&
...
function findBestMatch(candidates, searchSong, options = {}) {
  ...
  if (
    strictTitleMatch &&
    !bestMatch.details.isTitleMatch &&
    !bestMatch.details.areEquivalentTitles
  ) {
    return null;
  }
