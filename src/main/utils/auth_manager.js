...
import { getAuthInstance, updateConnectedServices } from "./firebase.js";
import {
  getSpotifyToken,
  setSpotifyToken,
  getAppleMusicToken,
...
function validateCredentials(clientId, clientSecret) {
  console.log("[Spotify] Credential validation:", {
  ...
  // More flexible validation - just check that we have reasonably long credentials
  return clientId?.length >= 20 && clientSecret?.length >= 20;
}
